...
//    }
        ...
        Color3f evalSheen(const BSDFQueryRecord &bRec) const {
            ...
            return C_sheen(bRec) * pow(1 - cosTheta_o, 5) * abs(cosTheta_o);
        }
...
//    float smithG_GGX_aniso(Vector3f w, float alpha_x, float alpha_y) const {
//        return 1 / (Frame::cosTheta(w) + sqrtf(w.x() * alpha_x * w.x() * alpha_x + w.y() * alpha_y * w.y() * alpha_y +
//                                               Frame::cosTheta(w) * Frame::cosTheta(w)));
//    }
//
//    float smith_GGX(Vector3f w, float alpha_g) const {
//        float a = alpha_g * alpha_g;
//        float b = Frame::cosTheta(w) * Frame::cosTheta(w);
//        return 1 / (Frame::cosTheta(w) + sqrtf(a + b - a * b));
//    }
...
//        }
//        Color3f Cspec0 = m_metallic * baseColor + (1 - m_metallic) * m_specular * .08 * (m_specularTint * Ctint + (1 - m_specularTint));
...
//        float Fd90 = 0.5f + 2 * m_roughness * wh_dot_wo * wh_dot_wo;
//        float Fd = lerp(FO, 1.f, Fd90) * lerp(FI, 1.f, Fd90);
//
//        // subsurface component
//        float Fss90 = m_roughness * wh_dot_wo * wh_dot_wo;
...
//        float Ds = Warp::squareToGTR2Pdf(wh, m_alpha_x, m_alpha_y) / Frame::cosTheta(wh);
...
//        return (INV_PI * lerp(m_subsurface, Fd, ss) * baseColor) * (1 - m_metallic)
//               + (Gs * Fs * Ds) / (4.f * cosTheta_o) + 0.25 * m_clearcoat * Gr * Fr * Dr / (4.f * cosTheta_o);
//    }

        float smith_g1(const Vector3f &v, const Vector3f &m) const {
            float xy_alpha_2 = m_alpha_x * v.x() * m_alpha_x * v.x() + m_alpha_y * v.y() * m_alpha_y * v.y(),
                    tan_theta_alpha_2 = xy_alpha_2 / (v.z() * v.z()),
                    result;

            result = 2.f / (1.f + sqrtf(1.f + tan_theta_alpha_2));
        ...
        float smith_ggx1(const Vector3f &v,  const Vector3f &wh,
            ...
            float result =
                    2.0f / (1.0f + sqrtf(1.0f + alpha_2 * tan_theta_2));
        ...
        Color3f calc_schlick(Color3f R0, float cos_theta_i, float eta) const {
            float eta_it, eta_ti;
            if (cos_theta_i >= 0.0f) {
                eta_it = eta;
                eta_ti = 1.f / eta;
            ...
            float cos_theta_t = sqrtf(cos_theta_t_sqr);
            if (eta_it > 1.0f) {
                return R0 * 1.0f + (Color3f(1.f) - R0) * schlick_weight(abs(cos_theta_i));
            } else {
                return R0 * 1.0f + (Color3f(1.f) - R0) * schlick_weight(cos_theta_t);
            }
        }

        float calc_schlick_float(float R0, float cos_theta_i, float eta) const {
            float eta_it, eta_ti;
            if (cos_theta_i >= 0.0f) {
                eta_it = eta;
                eta_ti = 1.f / eta;
            ...
            float cos_theta_t = sqrtf(cos_theta_t_sqr);
            if (eta_it > 1.0f) {
                return R0 * 1.0f + (1.f - R0) * schlick_weight(abs(cos_theta_i));
            } else {
                return R0 * 1.0f + (1.f - R0) * schlick_weight(cos_theta_t);
            }
        }
        ...
        Color3f principled_fresnel(const float &F_dielectric, const Color3f &base_color, const float &cos_theta_i, const float &eta) const {
            ...
            if (m_metallic > 0.f) {
                F_schlick += m_metallic * calc_schlick(
                        base_color, cos_theta_i, eta);
            }

            // Tinted dielectric component based on Schlick.
            if (m_specularTint > 0.f) {
                Color3f Ctint(1.f);
                if (base_color.getLuminance() > 0.0f) {
                    Ctint = base_color / base_color.getLuminance();
                }
                Color3f F0_spec_tint =
                        Ctint * schlick_R0_eta(eta_it);
                F_schlick +=
                        (1.0f - m_metallic) * m_specularTint *
                        calc_schlick(F0_spec_tint, cos_theta_i, eta);
            }

            // Front side fresnel.
            Color3f F_front =
                    (1.0f - m_metallic) * (1.0f - m_specularTint) * F_dielectric + F_schlick;
        ...
        virtual Color3f eval(const BSDFQueryRecord &bRec) const override {
            ...
            // diffuse component
            float wh_dot_wo = wh.dot(bRec.wo);
            float FO = SchlickFresnel(abs(cosTheta_o)); // FL
            float FI = SchlickFresnel(abs(cosTheta_i)); // FV
            float f_diff = (1.0f - 0.5f * FI) * (1.0f - 0.5f * FO);
            float Fd90 = 0.5f + 2 * m_roughness * wh_dot_wo * wh_dot_wo;
            float Fd = lerp(FO, 1.f, Fd90) * lerp(FI, 1.f, Fd90);
            float Rr = 2.0f * m_roughness * wh_dot_wo * wh_dot_wo;
            float f_retro = Rr * (FO + FI + FO * FI * (Rr - 1.0f));

            // subsurface component
            float Fss90 = Rr / 2.0f;
            float Fss = lerp(FO, 1.f, Fss90) * lerp(FI, 1.f, Fss90);
            float f_ss = 1.25 * (Fss * (1.f / (cosTheta_i + cosTheta_o) - 0.5) + 0.5);
            ...
            float FH = SchlickFresnel(wh_dot_wo);
            Color3f Fs = FH + (1 - FH) * Cspec0;
            float F_spec_dielectric = fresnel(wh.dot(bRec.wi), m_eta, 1.f);
            Color3f F_principled = principled_fresnel(F_spec_dielectric, baseColor,
                                                      wh.dot(bRec.wi), m_eta);
            float Gs = smith_g1(bRec.wi, wh) * smith_g1(bRec.wo, wh);

            // clearcoat component
            // include cosTheta_h already, need to cancel out
            float Dr = Warp::squareToGTR1Pdf(wh, m_alpha_g) / Frame::cosTheta(wh);
            float Fcc = calc_schlick_float(0.04f, wh.dot(bRec.wi), m_eta);
            float Gr = smith_ggx1(bRec.wi, wh, 0.25) * smith_ggx1(bRec.wo, wh, 0.25);

            // normalize like microfacet
            return (INV_PI * lerp(m_subsurface, f_diff + f_retro, f_ss) * baseColor * abs(cosTheta_o)) * (1 - m_metallic)
                   + (Gs * Fs * Ds) / (4.0f * abs(cosTheta_i)) + 0.25 * m_clearcoat * Gr * Fcc * Dr * abs(cosTheta_o);
        }
        ...
        virtual float pdf(const BSDFQueryRecord &bRec) const override {
            ...
            return weights[0] * pdfDiffuse(bRec) + weights[1] * pdfSpecular(bRec) + weights[2] * pdfClearcoat(bRec);
        }
